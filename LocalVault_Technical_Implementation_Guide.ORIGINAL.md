...
- **FR-103:** Save symbolic links as links without following them by default.
- **FR-104:** Save basic POSIX permissions and modification time.
...
- **FR-309:** Restore saved modification times and basic permissions.
...
- **NFR-006 — Portability:** Platform-specific code is isolated behind small POSIX abstractions.
...
- Windows-specific metadata and ACL support.
...
On Windows, the bootstrap command would be `bootstrap-vcpkg.bat`, but Windows support is not part of the first release.
...
- Build and run all tests on Linux and macOS before merging.
...
add_library(
    ...
    filesystem/posix_lock.cpp
    filesystem/posix_metadata.cpp
    storage/chunker.cpp
...
6. Platform-specific file locking and metadata code stays under `src/core/filesystem`.
...
- Skips unsupported special files with warnings.
...
If `worker_count == 0`, calculate a bounded default such as:

```cpp
std::clamp(std::thread::hardware_concurrency(), 1U, 16U)
```

Do not allow an unbounded worker count from user input.
...
The core library must not implement interactive prompts. For `OverwritePolicy::prompt`, the CLI or GUI resolves each conflict through an interface callback or converts the request into per-path decisions.
...
`source_device` and `source_inode` are optional diagnostic fields. They can later support hard-link preservation, but the first release does not recreate hard links.

For the source root itself, either omit an entry or represent it with `relative_path = ''`. Choose one policy and test it consistently. Recommended: represent the root directory with `relative_path = ''`.
...
4. Create `objects`, `temporary/objects`, `temporary/restores`, and `logs`.
5. Create `repository.lock`.
6. Acquire the exclusive writer lock.
7. Create `repository.db`.
8. Enable SQLite foreign keys, WAL, and full synchronous mode.
9. Run migration `001_initial_schema`.
10. Insert the `repository_info` row:
    - New random UUID.
...
12. Flush the database and containing directory when practical.
13. Release the writer lock.
14. Re-open the repository using the normal validation path as a final self-check.
...
10. For each regular file:
    - Capture pre-read metadata: size, modification time, mode, device, inode.
...
```text
before = {size, mtime_ns, inode, device}
read and process file
after  = {size, mtime_ns, inode, device}
```
...
- Invalid UTF-8 path under the first-release path policy.
...
- POSIX mode if metadata-only changes should be detected.
- Optional device/inode relationship where meaningful.
...
Document that metadata-based reuse assumes the filesystem updates modification times correctly.
...
The temporary directory must be on the same filesystem as `objects/` so rename remains atomic.
...
1. Open the repository read-only for queries, then acquire the writer lock only if restore bookkeeping or shared temporary paths require it. A simple first implementation may acquire the repository lock for the complete restore.
...
6. Apply overwrite policy.
7. Calculate a restore plan before writing.
...
11. Atomically rename it to the final destination according to overwrite policy.
...
- Both regular files and `file_hash` differs: `content_modified`.
- Same content hash but mode, mtime, or symlink metadata differs: `metadata_modified`.
...
10. Detect stale pending/failed snapshots.
...
Deletion requires an exclusive repository lock.

1. Validate the snapshot exists.
2. Prevent deletion of a `pending` snapshot being processed by the current operation.
3. Begin a transaction.
4. Delete the snapshot row.
5. Let `ON DELETE CASCADE` remove entries, chunk mappings, and warnings.
6. Commit.
7. Do not delete objects in the same command unless `--gc` is explicitly requested.

This keeps snapshot deletion fast and makes object cleanup independently recoverable.
...
3. Mark obviously stale `pending` snapshots as `failed` or delete their metadata.
4. Recommended first-release policy:
   - Preserve the snapshot row with failure information.
   - Delete its entries, mappings, and warnings.
5. Remove temporary object files older than a conservative threshold after confirming no operation owns the lock.
6. Remove stale restore temporary files created inside repository staging.
7. Leave orphan final objects for explicit GC.
8. Run a quick relationship check.
...
For Linux and macOS, implement a POSIX advisory lock with `flock` or `fcntl`.

Required behavior:

- Open/create `repository.lock`.
- Acquire exclusive non-blocking lock for write operations.
- Return `repository_busy` if another writer owns it.
- Keep the file descriptor open for the lock lifetime.
...
Readers can operate concurrently initially only if verified safe. A simpler first release can take the same exclusive lock for restore and verification, then relax locking later.
...
- Accept paths convertible to valid UTF-8.
- Skip non-convertible paths with a warning.
- Do not silently replace invalid bytes.
- Use exact stored path strings for identity.

On case-insensitive filesystems, detect destination collisions during restore.

## 25.2 Regular files

- Open in binary mode.
- Stream contents.
- Save size, mtime, and POSIX mode.
...
Catch `SIGINT` in the CLI, request cooperative stop, wait for safe cleanup, then return `130`.
...
Authoritative repository settings live in `repository_info` or a dedicated settings table.
...
The default production injector does nothing. Tests throw at selected points, reopen the repository, and validate recovery.
...
Purpose: scanner, path, SQLite, and metadata overhead.
...
Add a separate optional preset later. Do not combine ThreadSanitizer with AddressSanitizer in one binary.
...
Use vcpkg binary caching after the basic workflow works. Cache keys must include the vcpkg commit/baseline and triplet. Never reuse arbitrary binary caches across incompatible compilers or operating systems.
...
The macOS app bundle must include required Qt frameworks/plugins using the Qt deployment support appropriate to the installed Qt version. Validate the packaged app on a machine without the development Qt installation.
...
- Clean Linux and macOS builds pass.
- Tests and sanitizers pass.
- Full verification passes on a test repository.
- Package tested on a clean environment.
- Checksums generated.
...
Implement all commands, exit codes, JSON output, progress, signal cancellation, and clear errors.

Acceptance:

All core functions can be exercised without the GUI.
...
- [ ] Basic mode and mtime restore where supported.
...
- [ ] A release package runs on a clean target system.
...
- Paths must be valid UTF-8 under the first-release policy.
- POSIX ownership, ACLs, extended attributes, and sparse holes are not preserved.
...
- Long path support.
- Reparse points.
- Windows timestamps and attributes.
- ACL policy.
- Installer and Qt deployment.
- New CI jobs.

Do not claim Windows support until integration tests pass there.
...
## 42.11 Conflict resolver interface

Add this to the restore API when prompt mode is implemented:
...
Require a resolver when policy is `prompt`. The core invokes it synchronously; GUI adapters must marshal the question safely to the GUI thread.
